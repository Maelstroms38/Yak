...
class DetailViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        /* Setup Map */
        let geo = Strand!.objectForKey("location") as PFGeoPoint
        ...
        if(Strand?.objectForKey("comments") != nil) {
            comments = Strand?.objectForKey("comments") as? [String]
        }
        println(Strand)
        println(Strand?.objectForKey("text"))
        self.textLabel.text = Strand?.objectForKey("text") as? String
    ...
    func reply() {
        Strand?.addObject(commentView?.text, forKey: "comments")
        Strand?.saveInBackground()
